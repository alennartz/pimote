...
- `mobile/android/app/build.gradle.kts`, `mobile/android/app/src/main/AndroidManifest.xml` — app module build + manifest (registers `PimoteConnectionService`, `PimoteAuthenticatorService` with `CONTACTS_STRUCTURE` meta-data, `PimoteSyncAdapterService`, `MainActivity`, `InCallActivity`; declares `INTERNET` / `RECORD_AUDIO` / `MANAGE_OWN_CALLS` permissions; no `WRITE_CONTACTS` / `READ_CONTACTS` because contacts sync uses CALLER_IS_SYNCADAPTER + own-Account)
- `mobile/android/app/src/main/res/xml/account_authenticator.xml`, `mobile/android/app/src/main/res/xml/contacts.xml`, `mobile/android/app/src/main/res/xml/syncadapter.xml`, `mobile/android/app/src/main/res/values/strings.xml` — AccountManager account-type metadata, ContactsContract callable-MIME schema (`vnd.com.pimote.android.call`), and SyncAdapter shim metadata
