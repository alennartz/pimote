...
**Responsibilities:** command types (client→server), event types (server→client), response envelope, session state shape, message content model, multi-session commands (view_session, register_push, unregister_push, kill_conflicting_processes), multi-session events (session_conflict), enriched FolderInfo (activeSessionCount, activeStatus with working/idle/attention states), push subscription record types
...
- `server/src/server.ts` — HTTP server, static files, SPA fallback, /api/vapid-key endpoint, WebSocket upgrade
- `server/src/ws-handler.ts` — per-connection WebSocket handler, multi-session subscription tracking (subscribedSessions set, viewedSessionId), routes commands to sessions, session conflict detection, push registration, status change callbacks, extension UI response routing with session scoping
...
- `client/src/lib/stores/session-registry.svelte.ts` — reactive singleton wrapping SessionRegistry in $state, wires connection events to registry, switchToSession helper (sends view_session to server)
- `client/src/lib/stores/session-registry.test.ts` — SessionRegistry tests
- `client/src/lib/stores/connection.svelte.ts` — ConnectionStore: WebSocket lifecycle, per-session cursor tracking (sessionCursors map), subscribedSessions set with add/remove, reconnects all subscribed sessions on WebSocket reopen
...
- `client/src/routes/+page.svelte` — main page: session view (StatusBar, conflict banner, MessageList, ActiveSessionBar, InputBar) or landing (NotificationBanner, FolderList, ActiveSessionBar)
