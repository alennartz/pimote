...
- `server/src/event-buffer.ts` — ring buffer, SDK→wire event mapping (including buffered `tree_navigation_*` lifecycle events), streaming delta coalescing. Types against the real `AgentSessionEvent` union with an exhaustiveness guard (renamed/new SDK events fail at compile time, not silently); translates the SDK's `compaction_start`/`compaction_end` to pimote's wire `auto_compaction_*` and drops events with no wire representation (`queue_update`/`session_info_changed`/`thinking_level_changed`) rather than mis-emitting them. See DR-039
