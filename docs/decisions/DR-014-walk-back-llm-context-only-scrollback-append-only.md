...
- The extension depends on `ExtensionContext.abort()` and `message_update` events — both are stable pi-SDK surfaces (`@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts`). No pi-SDK changes required for v1.
