...
- **Orphaned type-level scaffolding.** Earlier exploratory commits had already landed an `attachment` content block and `images?: string[]` on `prompt`/`steer`/`follow_up` at the type level (unconsumed by any handler). This decision leaves them unused. They are harmless but should not be mistaken for a half-built expansion feature — there is intentionally no code path that expands `@` server-side.
